...
The initial selection SHALL be computed against the list the picker shows on
open, which is every loaded preset under the `All` scope filter with an empty
filter query. The active-preset status dot, the accent highlight on the active
card, and every navigation, filter, and scope behavior after the first frame
SHALL be unchanged: once the picker is open, the selection moves only in
