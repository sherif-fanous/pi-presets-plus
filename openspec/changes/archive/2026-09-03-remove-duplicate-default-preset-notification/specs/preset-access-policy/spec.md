...
the existing apply flow, capturing a fresh baseline, emitting the single visible
...
- **AND** exactly one visible success message naming the applied preset SHALL be
  emitted
- **AND** no additional success notification naming the same preset SHALL be
...
- **AND** the successful default activation SHALL emit only its single visible
  success message
