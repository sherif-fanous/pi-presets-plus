...
The `package.json` SHALL declare `@mariozechner/pi-coding-agent`,
`@mariozechner/pi-tui`, `@mariozechner/pi-ai`, and `@sinclair/typebox` as
...
- **WHEN** `package.json` is inspected
- **THEN** `peerDependencies` SHALL contain `@mariozechner/pi-coding-agent`,
  `@mariozechner/pi-tui`, `@mariozechner/pi-ai`, and `@sinclair/typebox`
...
- **AND** SHALL NOT include any files belonging to
  ...
  `@mariozechner/pi-ai`, or `@sinclair/typebox`
