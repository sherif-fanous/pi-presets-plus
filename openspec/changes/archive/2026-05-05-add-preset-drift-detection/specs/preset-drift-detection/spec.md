...
(no separating whitespace), and the appended `!` SHALL be rendered in the
...
`<name>` segment, both rendered in `dim`) SHALL be unchanged in either state.

#### Scenario: Active preset clean

- **WHEN** preset `plan` is active and `dirty` is `false`
- **THEN** the status bar entry SHALL be `preset: plan` (dim) with no trailing
  marker
