...
availability status when `unavailable`, and explicit shadowing status when
shadowed.
...
When the user presses `Enter` on a selected preset, the picker SHALL close and
SHALL invoke the existing apply flow for that preset. If apply refuses (preset
is `unavailable`), the picker SHALL stay open and the refusal notification SHALL
surface. Exact-name command activation (for example `/presets plan`) SHALL NOT
be part of the command surface; picker selection is the activation path.
...
- **THEN** the picker SHALL remain open and a notification SHALL describe why
...
page movement (`PgUp/PgDn`), scope cycle (`←/→`), and exit (`Esc`).
