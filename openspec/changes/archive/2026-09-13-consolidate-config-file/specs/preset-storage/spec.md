...
**Reason**: Version 2 configuration files replace the separate version 1 preset files.

**Migration**: The package automatically moves each valid legacy `presets` array into the matching scope's version 2 `config.json` during `session_start`.

### Requirement: Separate extension configuration file

**Reason**: Settings and presets now share the scoped version 2 configuration file, and project settings are supported.

**Migration**: The package automatically copies the user version 1 `showInactiveStatus` value into the version 2 user configuration. Project settings can be added to the new project configuration.
