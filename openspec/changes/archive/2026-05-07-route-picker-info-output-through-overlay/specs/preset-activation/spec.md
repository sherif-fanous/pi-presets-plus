...
outcome for each of model, thinking, and tools. The report SHALL distinguish at
least the following per-field outcomes:
...
- When clear is invoked from `/presets clear` typed at the prompt, the package
  SHALL deliver the report via `ctx.ui.notify` with severity `info`.
...
- **THEN** the report SHALL be delivered via `ctx.ui.notify` and SHALL name the
  cleared preset and indicate that model, thinking, and tools were restored
...
- **THEN** the report SHALL state that there is no active preset to clear,
  regardless of delivery surface
...
- `status` — print a textual summary of active state including baseline,
  ...
  attachment kind (`baseline` vs. `priorUnknown`). The summary SHALL be
  delivered via `ctx.ui.notify` (prompt invocation surface).
...
- **THEN** an info message SHALL state that no preset is active, delivered via
  `ctx.ui.notify`

#### Scenario: Status with baseline-managed attachment from prompt

- **WHEN** the user runs `/presets status` from the prompt and a preset is
  active with `restore.kind === "baseline"`
...
- **WHEN** the user runs `/presets status` from the prompt and a preset is
  active with `restore.kind === "unknown"`
- **THEN** the output SHALL be delivered via `ctx.ui.notify`
