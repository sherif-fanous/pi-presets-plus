...
`apply()` MAY still call `ctx.ui.notify` with severity `warning` for non-refusal
accompaniments (e.g. unknown-tools dropped during a successful apply). Warnings
ride alongside `ok: true` and are not part of the refusal return path.

Baseline capture rules, ownership rules, and `lastApplied` rules are unchanged
...
- **AND** `apply()` SHALL emit a `ctx.ui.notify(<text naming "bar">, "warning")`
  for the dropped tool
- **AND** `apply()` SHALL return `{ ok: true }`
