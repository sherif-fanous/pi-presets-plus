...
When the user presses `Enter` on a selected preset, the picker SHALL close and SHALL invoke the existing apply flow for that preset. If apply refuses (preset is `unavailable`), the picker SHALL stay open and the refusal notification SHALL surface. Exact-name command activation (for example `/presets plan`) SHALL NOT be part of the command surface; picker selection is the activation path.
...
#### Scenario: Activate unavailable preset

- **WHEN** the user selects a preset marked `unavailable` and presses `Enter`
- **THEN** the picker SHALL remain open and a notification SHALL describe why activation was refused
...
The picker SHALL render a footer hint row using readable title-case action labels and showing at minimum: activate (`⏎`), filter (`/`), movement (`↑/↓`), page movement (`PgUp/PgDn`), scope cycle (`←/→`), and exit (`Esc`).
