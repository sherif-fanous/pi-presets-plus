...
The `preset-drift-detection` capability makes the preset footer badge tell the truth about whether Pi's live state still matches the active preset's declared fields. When the user manually changes the model, thinking level, or active tools after applying a preset, drift detection flips an in-memory `dirty` flag on the active attachment, surfaces a warning-colored `!` in the footer badge, and shows a `Drift:` row on the picker card for the active preset. The preset stays attached — instructions still inject, `clear` still works — and the user resolves drift by either manually returning state to the preset's declared values (dirty clears) or re-applying the preset from the picker (dirty clears and state snaps back). Model drift is detected immediately on `model_select`, thinking drift immediately on `thinking_level_select`, and all three dimensions are rechecked at every `turn_start` as a safety net (also the primary detection path for tool changes). Drift detection is in-memory only: it compares current Pi values against a snapshot captured on the active state at apply / restore time and never re-reads the on-disk preset files per turn. The dirty flag does not persist across sessions; restored attachments start clean and the next `turn_start` re-evaluates.
