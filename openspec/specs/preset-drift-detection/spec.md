# preset-drift-detection Specification

## Purpose

The `preset-drift-detection` capability makes the preset footer badge tell the
truth about whether Pi's live state still matches the active preset's declared
fields. When the user manually changes the model, thinking level, or active
tools after applying a preset, drift detection flips an in-memory `dirty` flag
on the active attachment, surfaces a warning-colored `!` in the footer badge,
and shows a `Drift:` row on the picker card for the active preset. The preset
stays attached — instructions still inject, `clear` still works — and the user
resolves drift by either manually returning state to the preset's declared
values (dirty clears) or re-applying the preset from the picker (dirty clears
...
- **THEN** the active preset SHALL be unset and the standard clear behavior
  SHALL apply
...
- **WHEN** preset `plan` is active (clean) and the user selects a different
  model via `/model`
...
- **WHEN** preset `plan` is active (clean) and the user cycles to a different
  model
- **THEN** the active preset's `dirty` flag SHALL become `true`
...
- **WHEN** the active preset is currently dirty due to a model change and the
  user manually selects the preset's recorded model
...
- **WHEN** preset `plan` is active (clean) and the user selects a thinking level
  that no longer matches the preset's effective thinking level
- **THEN** the active preset's `dirty` flag SHALL become `true` immediately on
  `thinking_level_select`
- **AND** the active preset SHALL remain attached

#### Scenario: Manual tool toggle on a preset that declares tools

- **WHEN** preset `plan` (with non-empty `tools`) is active (clean), the next
  turn starts, and `pi.getActiveTools()` no longer matches the preset's `tools`
  as a set
- **THEN** the active preset's `dirty` flag SHALL become `true`
...
- **WHEN** the active preset is dirty due to thinking-level drift and the user
  manually returns the thinking level to the preset's effective value
- **THEN** the active preset's `dirty` flag SHALL return to `false` immediately
  on `thinking_level_select`
...
- **WHEN** the preset declares `tools: ["read", "bash"]` and
  `pi.getActiveTools()` returns `["bash", "read"]`
...
- **WHEN** the active preset is dirty and the user selects that preset in the
  picker and presses `Enter`
- **THEN** the apply runs, captures a fresh snapshot, and SHALL set
  `dirty: false`
...
When the active preset is dirty, the footer status entry under the key
`presets-plus` SHALL append a single `!` character immediately after `<name>`
with no separating whitespace, and the appended `!` SHALL be rendered in the
theme's `warning` color. When the active preset is clean, the entry SHALL omit
the `!` and match the format defined by the `preset-activation` capability
verbatim. The remainder of the badge format, the `Preset: ` prefix and the
`<name>` segment, SHALL be unchanged in either state and both SHALL be rendered
in `dim`.

#### Scenario: Active preset clean

- **WHEN** preset `plan` is active and `dirty` is `false`
- **THEN** the status bar entry SHALL be `Preset: plan` with dim styling and no
  trailing marker

#### Scenario: Active preset dirty

- **WHEN** preset `plan` is active and `dirty` is `true`
- **THEN** the status bar entry SHALL render the dim text `Preset: plan`
  followed immediately by a `!` rendered in the theme's `warning` color
...
When the active preset is dirty, the picker SHALL render a `Drift:` row on the
active preset's card. The row's value SHALL match the format
`⚠ Dirty — <reasons> differ` and SHALL be rendered in the theme's `warning`
color, mirroring the existing `Status:` rows used by clamp warnings and
unavailability. `<reasons>` SHALL be the comma-separated names of fields that
differ (any subset of `model`, `thinking level`, `tools`). When the active
preset is clean, the picker SHALL omit the `Drift:` row.
...
- **WHEN** the active preset is dirty because the model has drifted and the
  picker is open
...
- **WHEN** the active preset is dirty because both thinking level and tools have
  drifted and the picker is open
- **THEN** the active preset's card SHALL show a `Drift:` line listing both
